"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.HUDPanel = exports.createQuestHUD = void 0;
const ecs_1 = require("@dcl/sdk/ecs");
const math_1 = require("@dcl/sdk/math");
const react_ecs_1 = require("@dcl/sdk/react-ecs");
const QuestComponent = ecs_1.engine.defineComponent('dcl:quests:QuestComponent', {
    name: ecs_1.Schemas.String,
    steps: ecs_1.Schemas.Array(ecs_1.Schemas.Map({
        id: ecs_1.Schemas.String,
        description: ecs_1.Schemas.String,
        tasks: ecs_1.Schemas.Array(ecs_1.Schemas.Map({
            done: ecs_1.Schemas.Boolean,
            description: ecs_1.Schemas.String
        }))
    })),
    nextSteps: ecs_1.Schemas.Array(ecs_1.Schemas.String),
    done: ecs_1.Schemas.Boolean
});
function createQuestHUD(opts) {
    let state = {
        hideHUD: false,
        selectedStepId: null,
        rawEntity: null,
        quest: null
    };
    function getState() {
        return { ...state };
    }
    function setState(newState) {
        state = {
            ...state,
            ...newState
        };
    }
    if (opts?.autoRender) {
        react_ecs_1.ReactEcsRenderer.setUiRenderer(() => (0, exports.HUDPanel)([getState, setState], opts));
    }
    function getQuest() {
        return [...ecs_1.engine.getEntitiesWith(QuestComponent)][0][1];
    }
    return {
        upsert: (instance) => {
            const questUIData = generateQuestUI(instance);
            if (state.rawEntity) {
                QuestComponent.createOrReplace(state.rawEntity, {
                    ...questUIData
                });
                state.quest = getQuest();
            }
            else {
                const questEntity = ecs_1.engine.addEntity();
                QuestComponent.create(questEntity, { ...questUIData });
                state.rawEntity = questEntity;
                state.quest = getQuest();
            }
        },
        getHUDComponent: () => () => (0, exports.HUDPanel)([getState, setState], opts),
        render: () => react_ecs_1.ReactEcsRenderer.setUiRenderer(() => (0, exports.HUDPanel)([getState, setState], opts)),
        updateOptions: (newOptions) => react_ecs_1.ReactEcsRenderer.setUiRenderer(() => (0, exports.HUDPanel)([getState, setState], { ...opts, ...newOptions })),
        getHUDComponentWithUpdatedOptions: (newOptions) => () => (0, exports.HUDPanel)([getState, setState], { ...opts, ...newOptions })
    };
}
exports.createQuestHUD = createQuestHUD;
const defaultHudBox = {
    flexDirection: 'column',
    justifyContent: 'flex-start',
    margin: { left: 10 },
    positionType: 'absolute',
    position: { top: '28%' }
};
const HUDPanel = (handlers, props) => {
    const [getState, setState] = handlers;
    const { selectedStepId: showTask, hideHUD, quest } = getState();
    return (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
            ...defaultHudBox,
            ...(props?.leftSidePanel || {})
        } },
        !showTask && quest ? QuestProgress(quest, handlers, props) : StepTasks(handlers, props),
        quest ? (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: { display: 'flex' } }, !showTask ? (react_ecs_1.default.createElement(react_ecs_1.Button, { value: hideHUD ? 'Show Quest Progress' : 'Hide', variant: "primary", uiTransform: {
                width: !hideHUD ? 80 : 120,
                height: !hideHUD ? 20 : 30,
                margin: {
                    top: 5
                }
            }, onMouseDown: () => setState({ ...getState(), hideHUD: !hideHUD }), ...props?.showHideToggleButton })) : (react_ecs_1.default.createElement(react_ecs_1.Button, { value: "Close", variant: "primary", uiTransform: {
                width: 80,
                height: 20,
                margin: {
                    top: 5
                }
            }, onMouseDown: () => setState({ ...getState(), selectedStepId: null }), ...props?.closeTasksBoxButton })))) : null));
};
exports.HUDPanel = HUDPanel;
const QuestProgress = (quest, handlers, stylingOpts) => {
    const [getState] = handlers;
    return (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
            display: getState().hideHUD ? 'none' : 'flex',
            flexDirection: 'column',
            padding: 10,
            minWidth: 180,
            maxWidth: 400,
            width: 180,
            ...stylingOpts?.questBox?.uiTransform
        }, uiBackground: {
            color: math_1.Color4.create(0, 0, 0, 0.9),
            ...stylingOpts?.questBox?.uiBackground
        } },
        react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: { margin: { bottom: 10 }, ...stylingOpts?.questNameContainer?.uiTransform } },
            react_ecs_1.default.createElement(react_ecs_1.Label, { textAlign: "middle-left", fontSize: 13, value: `Quest: ${quest.name}`, ...stylingOpts?.questNameContainer?.label })),
        !quest.done ? (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
                flexDirection: 'column',
                padding: { top: 10, bottom: 10 },
                ...stylingOpts?.stepsContainer?.uiTransform
            } }, [...quest.steps].map((s) => CurrentStep(s, handlers, stylingOpts)))) : (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
                flexDirection: 'column',
                padding: { top: 10, bottom: 10 },
                ...stylingOpts?.questCompletionLabel?.uiTransform
            } },
            react_ecs_1.default.createElement(react_ecs_1.Label, { uiTransform: {
                    margin: { bottom: 10 }
                }, textAlign: "middle-center", fontSize: 12, value: "Quest is completed!", ...stylingOpts?.questCompletionLabel?.label }))),
        quest.nextSteps.length > 0 && !quest.done ? (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
                display: 'flex',
                flexDirection: 'column',
                margin: { bottom: 10 },
                ...stylingOpts?.nextSteps?.nextTitleUiEntity
            } },
            react_ecs_1.default.createElement(react_ecs_1.Label, { uiTransform: {
                    padding: { bottom: 10 },
                    margin: { bottom: 10 },
                    ...stylingOpts?.nextSteps?.nextTitleProps?.uiTransform
                }, textAlign: "middle-left", fontSize: 12, value: "Next: ", ...stylingOpts?.nextSteps?.nextTitleProps }),
            [...quest.nextSteps].map((next) => (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: { margin: { bottom: 5 }, ...stylingOpts?.nextSteps?.labels?.labelUiEntity } },
                react_ecs_1.default.createElement(react_ecs_1.Label, { textAlign: "middle-left", fontSize: 11, value: next, ...stylingOpts?.nextSteps?.labels?.props })))))) : null));
};
const CurrentStep = (step, handlers, stylingOpts) => {
    return (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: { flexDirection: 'column', ...stylingOpts?.stepsContainer?.labels?.labelUiEntity } },
        react_ecs_1.default.createElement(react_ecs_1.Label, { uiTransform: {
                overflow: 'hidden',
                padding: { top: 10, bottom: 10 },
                ...stylingOpts?.stepsContainer?.labels?.props?.uiTransform
            }, textAlign: "middle-left", fontSize: 12, value: `Step: ${step.description}`, ...stylingOpts?.stepsContainer?.labels?.props }),
        react_ecs_1.default.createElement(ShowTasksButton, { stepId: step.id, handlers: handlers, stylingOpts: stylingOpts })));
};
const ShowTasksButton = ({ stepId, handlers, stylingOpts }) => {
    const [getState, setState] = handlers;
    return (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: { display: 'flex', ...stylingOpts?.stepsContainer?.showTasksButton?.buttonUiEntity } },
        react_ecs_1.default.createElement(react_ecs_1.Button, { value: "Show Tasks", variant: "primary", uiTransform: { width: 80, height: 20, margin: 4 }, onMouseDown: () => {
                setState({ ...getState(), selectedStepId: stepId });
            }, ...stylingOpts?.stepsContainer?.showTasksButton?.buttonProps })));
};
const StepTasks = ([getState], stylingOpts) => {
    const { selectedStepId, quest } = getState();
    return (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
            flexDirection: 'column',
            padding: 10,
            minWidth: 180,
            maxWidth: 400,
            width: 180,
            ...stylingOpts?.tasksBox?.uiTransform
        }, uiBackground: {
            color: math_1.Color4.create(0, 0, 0, 0.9),
            ...stylingOpts?.tasksBox?.uiBackground
        } }, selectedStepId != null && quest != null
        ? quest.steps
            .find((s) => s.id === selectedStepId)
            ?.tasks.map((t) => Task(t, stylingOpts))
        : null));
};
const Task = (task, stylingOpts) => (react_ecs_1.default.createElement(react_ecs_1.UiEntity, { uiTransform: {
        flexDirection: 'column',
        margin: { bottom: 10 },
        ...stylingOpts?.tasksBox?.labels?.labelUiEntity
    } },
    react_ecs_1.default.createElement(react_ecs_1.Label, { textAlign: "middle-left", fontSize: 12, value: `${task.done ? 'DONE: ' : 'TODO: '} ${task.description}`, ...stylingOpts?.tasksBox?.labels?.props })));
function generateQuestUI(questInstance) {
    const steps = [];
    const nextSteps = [];
    if (questInstance.quest.definition?.steps) {
        for (const step of questInstance.quest.definition?.steps) {
            if (questInstance.state.currentSteps[step.id]) {
                const content = questInstance.state.currentSteps[step.id];
                steps.push({
                    id: step.id,
                    description: step.description,
                    tasks: step.tasks.map((task) => {
                        return {
                            description: task.description,
                            done: !!content.tasksCompleted.find((t) => t.id == task.id)
                        };
                    })
                });
                nextSteps.push(...questInstance.quest.definition?.connections
                    .filter((conn) => conn.stepFrom === step.id)
                    .map((conn) => questInstance.quest.definition?.steps.find((step) => step.id === conn.stepTo)?.description ||
                    '(Step without description)'));
            }
        }
    }
    return { name: questInstance.quest.name, steps, nextSteps, done: questInstance.state.stepsLeft == 0 };
}
//# sourceMappingURL=data:application/json;base64,